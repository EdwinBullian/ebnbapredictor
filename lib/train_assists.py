...
# --- Minutes prediction features (subset that doesn't leak AST info) ---
MINUTES_FEATURE_COLS = [
    ...
    # Career baseline
    "ast_career_min_avg", "ast_career_games",
    # Game context
...
FEATURE_COLS = [
    ...
    "minutes_trend", "minutes_std_last_5",
    "min_ewm_10", "min_ewm_5", "min_momentum",
    # Team/opponent (real stats)
    "opp_def_rating", "opp_pace", "game_pace", "usage_rate",
    "spread_proxy", "blowout_risk",
    "opp_ast_allowed", "opp_tov_forced", "opp_ast_opportunity",
    # Schedule
...
MODEL_PATH = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "models", "xgb_assists_model.joblib")


class EnsembleModel:
    """Weighted ensemble of XGBoost, LightGBM, and Ridge regression for assists."""
    ...
    def predict(self, X):
        ...
        if "predicted_minutes" not in X.columns:
            ...
            if len(min_cols_available) >= 5:
                ...
                X["predicted_minutes"] = X.get("avg_minutes_last_5", 28)
...
def train_model(features_df):
    """Train ensemble model for assists prediction."""
...
if __name__ == "__main__":
    from lib.data_collection import get_top_assisters, collect_all_game_logs
    from lib.features import (build_assist_features, add_opponent_assist_features,
                              add_player_advanced_features)

    print("=== NBA Assists Predictor — Training ===\n")

    print("Step 1: Getting top 100 assist leaders...")
    assisters = get_top_assisters(season="2025-26", top_n=100)
    ...
    logs = collect_all_game_logs(assisters, cache_path="data/game_logs/all_logs_ast.csv", seasons=seasons)
    ...
    features = add_opponent_assist_features(features, season="2025-26")
    print("  + Opponent features (def rating, pace, assist opportunity)")
    features = add_player_advanced_features(features, season="2025-26")
    print("  + Player advanced (usage rate, interactions)")

    print(f"  Final: {len(features)} rows, {len(FEATURE_COLS)} assist features\n")

    print("Step 4: Training model...")
    model, metrics = train_model(features)

    print(f"\n  ========== RESULTS ==========")
    print(f"  Ensemble MAE:     {metrics['mae']:.2f} assists")
