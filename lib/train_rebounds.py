...
# --- Minutes prediction features (subset that doesn't leak REB info) ---
MINUTES_FEATURE_COLS = [
    ...
    # Career baseline
    "reb_career_min_avg", "reb_career_games",
    # Game context
...
FEATURE_COLS = [
    ...
    "minutes_trend", "minutes_std_last_5",
    "min_ewm_10", "min_ewm_5", "min_momentum",
    # Team/opponent (Improvement #2 — real stats)
    "opp_def_rating", "opp_pace", "game_pace", "usage_rate",
    "spread_proxy", "blowout_risk",
    "opp_reb_allowed", "opp_oreb_allowed", "opp_dreb_allowed",
    "opp_fg_pct_allowed", "opp_reb_opportunity",
    "opp_pos_reb_allowed",
    # Schedule
...
MODEL_PATH = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "models", "xgb_rebounds_model.joblib")


class EnsembleModel:
    """Weighted ensemble of XGBoost, LightGBM, and Ridge regression for rebounds."""
    ...
    def predict(self, X):
        ...
        if "predicted_minutes" not in X.columns:
            ...
            if len(min_cols_available) >= 5:
                ...
                X["predicted_minutes"] = X.get("avg_minutes_last_5", 25)
...
def train_model(features_df):
    """Train ensemble model for rebounds prediction."""
...
if __name__ == "__main__":
    from lib.data_collection import get_top_rebounders, collect_all_game_logs
    from lib.features import (build_rebound_features, add_opponent_rebound_features,
                              add_player_advanced_features, add_position_defense)

    print("=== NBA Rebounds Predictor — Training ===\n")

    print("Step 1: Getting top 100 rebounders...")
    rebounders = get_top_rebounders(season="2025-26", top_n=100)
    ...
    logs = collect_all_game_logs(rebounders, cache_path="data/game_logs/all_logs_reb.csv", seasons=seasons)
    ...
    features = add_opponent_rebound_features(features, season="2025-26")
    print("  + Opponent features (def rating, pace, rebound opportunity)")
    features = add_player_advanced_features(features, season="2025-26")
    print("  + Player advanced (usage rate, interactions)")

    print("  + Position defense matchup (fetching rosters)...")
    try:
        ...
        print("  + Position rebound defense added")
    except Exception as e:
        print(f"  ! Position defense skipped: {e}")
        features["opp_pos_reb_allowed"] = 13.0

    print(f"  Final: {len(features)} rows, {len(FEATURE_COLS)} rebound features\n")

    print("Step 4: Training model...")
    model, metrics = train_model(features)

    print(f"\n  ========== RESULTS ==========")
    print(f"  Ensemble MAE:     {metrics['mae']:.2f} rebounds")
